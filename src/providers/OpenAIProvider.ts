...
export class OpenAIProvider implements IAIProvider {
  ...
  isConfigured(): boolean {
    return this.settings.providers.openai.apiKey.trim().length > 0;
  }
  ...
  async testConnection(): Promise<ConnectionResult> {
    ...
    try {
      const resp = await requestUrl({
        ...
        body: JSON.stringify({
          model: this.settings.providers.openai.model,
          max_tokens: 10,
  ...
  private headers(): Record<string, string> {
    return {
      ...
      Authorization: `Bearer ${this.settings.providers.openai.apiKey}`,
    };
  }
}
